...
int POSIXTimerSetMs(timer_t timerid, int interval_msec, int msec)
{
    /* 第一次间隔it.it_value这么长,以后每次都是it.it_interval这么长,就是说it.it_value变0的时候会>装载it.it_interval的值 */
    struct itimerspec it;
    it.it_interval.tv_sec = interval_msec / 1000; // 回调函数执行频率为1s运行1次
    it.it_interval.tv_nsec = interval_msec * 1000000;
    it.it_value.tv_sec = msec / 1000; // 倒计时3秒开始调用回调函数
    it.it_value.tv_nsec = msec * 1000000;
