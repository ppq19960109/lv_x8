...
static void open_page_anim(lv_obj_t * obj)
{
    ...
    if(page->page_update_cb != NULL) page->page_update_cb(page);
...
static void close_page_anim(lv_obj_t * obj)
{
    ...
    if(page->page_close_cb != NULL) page->page_close_cb(page);
...
static void property_change_cb(const char * key, void * value)
{
    ...
    if(strcmp("LStOvState", key) == 0) {
        ...
        for(int i = 0; i < arraySize; i++) {
            ...
            if(strcmp(ssid->valuestring, "") == 0) continue;
        ...
        if(g_wifiPageStatus == 0) {
            if(strlen(g_wifi_info.ssid) > 0) memset(g_wifi_info.ssid, 0, sizeof(g_wifi_info.ssid));
        }
    ...
    if(page_property_change_cb != NULL) page_property_change_cb(key, value);
}
...
static void home_bar_event_cb(lv_event_t * e)
{
    ...
    switch(user_data) {
        case 0:
            ...
            break;
        case 1: lv_100ask_page_manager_set_open_page(NULL, "page_set"); break;
        case 2: {
        } break;
        case 3: {
            if(lv_page_get_page_depth() > 1) {
