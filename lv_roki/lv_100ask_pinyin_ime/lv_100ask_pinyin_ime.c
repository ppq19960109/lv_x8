...
static void set_text_sel(lv_obj_t * kb,lv_100ask_pinyin_ime_t * pinyin_ime)
{
    lv_obj_t *ta=lv_keyboard_get_textarea(kb);
    int pos =lv_textarea_get_cursor_pos(ta);
    lv_obj_t * label =lv_textarea_get_label(ta);

    lv_label_set_text_sel_start(label, pos-pinyin_ime->ta_count);
    lv_label_set_text_sel_end(label, pos);
    printf("%s,pos:%d ta_count:%d\n",__func__,pos,pinyin_ime->ta_count);
}
static void lv_100ask_pinyin_ime_event(lv_event_t * e)
{
...
static void pinyin_page_proc(lv_obj_t* obj, uint16_t dir)
{
    ...
    for(uint8_t i = 0; (i < pinyin_ime->cand_num && i < LV_100ASK_PINYIN_IME_CAND_TEXT_NUM); i++)
    {
        ...
        for(uint8_t j = 0; j < 3; j++)
        {
            // printf("%s,i:%d,j:%d cand_str:%p\n",__func__,i,j,pinyin_ime->cand_str);
            if(pinyin_ime->cand_str!=NULL)
